...
如我们所见，在通用语言中——甚至在数学教科书里——我们已经学会了理解这些微妙之处时加入确切的语义，可能是无意识的，除非给出一个像这样的例子指明这一点。我们对「$A$ 是 $B$」解释，首先是断言 $A$ 的存在，然后以此为大前提，剩下的句子被理解为取决于这个前提。换言之，在通用语言中，动词「是（Is）」意味着主语与宾语的区别，形式逻辑或传统数学中所用的符号「=」却没有这种用法。（不过，在计算机语言中，我们遇到过诸如「\type{J = J + 1}」这样的语句，似乎都理解这样的语句，但是其中的符号「=」已经隐含了这种区别。）
...
通用语言——或者，至少，英语——有一个近乎普遍的倾向，对认识论意义上的陈述进行伪装，方法是把它置入合乎语法的形式，冒充本体论意义的描述，从而具备了欺骗性。当前的概率论中，错误的主要根源在于欠缺思考而忽略了这一事实。用本体论的方式来解释第一种陈述，如同宣称一个人私下的想法和感觉在自然界中是外在存在的事实。我们将这种现象称为「心灵投射谬误」，并注意到它如何再三制造麻烦。但是这种麻烦并不仅仅局限于概率论；这种麻烦一经指出，就会清楚地看到，哲学家和格式塔心理学家的大部分论述，以及物理学家在解释量子力学方面的尝试，皆可归结作者重复陷入思想透射谬误所致的荒谬之谈。

这些例子说明了当我们尝试将通用语言的复杂语句翻译为形式逻辑的简单语句时务必谨慎。当然，通用语言的严谨性通常不如我们在形式逻辑中所期望的那样。但是每个人注意并警惕到这一点，这样风险性就会很小。
