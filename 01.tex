...
因而，两种运算 $(\text{与，非})$ 就足以构成演绎逻辑的完备集\footnote{需要思考：这是否意味着，写任何一个计算机程序，只需要两条命令？}。在确定可信推理规则的完备集时，这一事实非常重要；见第 \in[quantitive-rules] 章。
...
现在，我们将注意力转向我们对逻辑所作的扩展。这一扩展遵循下面要讨论的一些条件。我们将这些条件称为「祈求」而非「公理」，因为它们并不断言何事为真，而是仅陈述怎样才是令人满意的目标。这些目标是否能够无矛盾地达成，是否确定任何唯一的逻辑扩展，是数学分析上的事情，而这些分析在第 \in[quantitive-rules] 章给出。
...
\startformula
\text{(\convertnumber{R}{1})}\quad\text{\it 可信度用实数表示。}
\stopformula

实际上，祈求 (\convertnumber{R}{1}) 是强加在我们身上的，因为机器人的大脑必须通过执行一些明确的物理过程方能运转。不过，这也是理论上的需要（见附录 A）；任何具有一致性的理论，不可能不具备与祈求 (\convertnumber{R}{1}) 等价的性质。
...
\startformula
\text{(\convertnumber{R}{2})}\quad\text{\it 定性符合常识。}
\stopformula
...
\placeformula
\startformula
\text{(\convertnumber{R}{3a})}\quad\hbox to .7\hsize{\startmathmatrix[n=2, align={left}]
...
\startformula
\text{(\convertnumber{R}{3a})}\quad\hbox to .7\hsize{\startmathmatrix[n=3, align={left}]
\NC \text{\it 机器人总是会考虑所有与问题相关的证据。它不会随意忽略某些}\NR
\NC \text{\it 信息，仅基于剩下的信息给出结论。换言之，这个机器人是完全}\NR
\NC \text{\it 非意识形态化的。}\NR
\stopmathmatrix}
\stopformula
