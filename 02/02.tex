...
结果得到了 (\in[product-rule])，某种意义上，它是公设 (\convertnumber{R}{3}a) 所述一致性的必要条件。不过，对于任意多个联结的命题，(\in[product-rule]) 显然也能保证这种一致性的存在。例如，用 (\in[expand-way]) 的办法可以将 $(ABCDEFG|H)$ 展开为数目繁多的不同形式；但是，只要 (\in[product-rule]) 成立，这些形式会产生同一个结果。
